...
from tcn import TCN, tcn_full_summary  # Import the TCN layer from keras-tcn

# Suppress the deprecation warning for plt.show() usage
st.set_option('deprecation.showPyplotGlobalUse', False)

def multi_TCN(temp, target_columns):
    model = Sequential()
    model.add(InputLayer((7, temp.shape[1])))
    model.add(TCN(64, kernel_size=2, activation='relu'))
    model.add(Flatten())
